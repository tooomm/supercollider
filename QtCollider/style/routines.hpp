...
template<typename ShapeT> void drawSunken (QPainter *p, const QPalette &plt, const ShapeT &shape,
                                           const QColor &color, const QColor &focusColor = QColor())
{
  p->save();
  ...
  QColor c2=color; c2.setAlpha(150);
  p->setPen(Qt::NoPen);
  p->setBrush(c2);
  shape.draw( p, shape._rect.adjusted(2,2,-2,-2) );
...
template<typename RectT> RectT sunkenContentsRect ( const RectT & r )
{
  return r.adjusted(1, 1,-1,-1 );
}

template<typename RectT>
qreal xValue ( qreal x, const RectT & bounds )
{
  return (x - bounds.x()) / bounds.width();
}

template<typename RectT>
qreal yValue ( qreal y, const RectT & bounds )
{
  return (bounds.y() + bounds.height() - y) / bounds.height();
}

template<typename RectT, typename SizeT>
qreal xValue ( qreal x, const RectT & bounds, const SizeT & margin )
{
  return margin.width() < bounds.width() ?
    (x - bounds.x() - (margin.width() * 0.5)) /
    (bounds.width() - margin.width())
    : 0.0;
}

template<typename RectT, typename SizeT>
qreal yValue ( qreal y, const RectT & bounds, const SizeT & margin )
{
  return margin.height() < bounds.height() ?
    (bounds.y() + bounds.height() - y - (margin.height() * 0.5)) /
    (bounds.height() - margin.height())
    : 0.0;
}

template<typename RectT, typename PointT>
QPointF value ( const PointT & pos, const RectT & bounds )
{
...
template<typename RectT, typename SizeT>
RectT marginsRect ( const RectT & bounds, const SizeT & margin )
{
  RectT r;
...
template<typename RectT, typename SizeT>
RectT rect ( const QPointF & value, const RectT & bounds, const SizeT & size )
{
...
template<typename RectT, typename SizeT>
QPointF pos ( const QPointF & value, const RectT & bounds, const SizeT & margin )
{
...
template<typename RectT>
QPointF pos ( const QPointF & value, const RectT & bounds )
{
