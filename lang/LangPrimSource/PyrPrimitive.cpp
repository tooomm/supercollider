...
static int prLanguageConfig_getLibraryPaths(struct VMGlobals * g, int numArgsPushed, int pathType)
{
	...
	for (size_t i = 0; i != numberOfPaths; ++i) {
		PyrString * pyrString = newPyrString(g->gc, SC_Filesystem::pathAsUTF8String(dirVector[i]).c_str(), 0, true);
...
{
	PyrSlot *a = g->sp;
	PyrString* str = newPyrString(g->gc, SC_Filesystem::pathAsUTF8String(gLanguageConfig->getConfigPath()).c_str(), 0, false);
    if(str->size == 0) {
        SetNil(a);
    } else {
        SetObject(a, str);
